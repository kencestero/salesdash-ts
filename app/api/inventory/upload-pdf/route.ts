...
PRICING LOGIC ($2,000 MINIMUM PROFIT RULE):
...
3. **Calculate desired salePrice using MINIMUM $2,000 PROFIT RULE**:
   - First calculate: standardPrice = cost × 1.015 (1.50% markup)
   - Then calculate profit: profit = standardPrice - cost
   - IF profit < $2,000 THEN salePrice = cost + $2,000
...
EXAMPLES:
- PRICE=$4,568: standardPrice=$4,636.52, profit=$68.52 (<$2,000) → salePrice=$6,568 ($4,568+$2,000)
- PRICE=$150,000: standardPrice=$152,250, profit=$2,250 (>$2,000) → salePrice=$152,250 (use 1.50%)
- VIN 96193: PRICE=$10,230, NEW DISC'D PRICE="MAKE OFFER" → cost=$10,230, makeOffer=true, salePrice=$12,230
