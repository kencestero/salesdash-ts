...
/**
 * Enhanced CRM Dashboard View
 * Manager command center with charts, analytics, and exports
 */
...
import { RefreshCw, Users, Flame, TrendingUp, Clock, Download, BarChart3 } from "lucide-react";
...
import { TemperatureChart } from "@/components/crm/dashboard/temperature-chart";
import { PriorityChart } from "@/components/crm/dashboard/priority-chart";
import { DateRangeFilter } from "@/components/crm/dashboard/date-range-filter";
import {
...
export default function EnhancedDashboardView({ session }: DashboardViewProps) {
  ...
  const handleExportSummary = () => {
    if (data) {
      exportDashboardSummaryCSV(data);
      toast({
        title: "Export Complete",
        description: "Dashboard summary downloaded as CSV",
      });
    }
  };
  ...
  const handleExportTeam = () => {
    if (data) {
      ...
      toast({
        title: "Export Complete",
        description: "Team performance downloaded as CSV",
      });
    }
  };
  ...
  const handleExportHotLeads = () => {
    if (data) {
      exportHotLeadsCSV(data.hotLeads);
      toast({
        title: "Export Complete",
        description: "Hot leads downloaded as CSV",
      });
    }
  };
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Your team's performance with advanced insights
          </p>
        </div>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="analytics" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <ConversionFunnel leadsByStatus={data.leadsByStatus} />
            <TemperatureChart leadsByTemperature={data.leadsByTemperature} />
          </div>

          {/* Charts Row 2 */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <PriorityChart leadsByPriority={data.leadsByPriority} />
